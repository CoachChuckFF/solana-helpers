...
const main = async () => {
  ...
  let SFT = await helpers.createSFT(provider, helpers.TEST_SPL_URL, helpers.TEST_SPL_METADATA, new anchor.BN(100));

  console.log('Updating SFT...');
  SFT = await helpers.updateSFT(provider, SFT.mint, helpers.TEST_SPL_METADATA, true);

  console.log(await helpers.getMetadataAccount(provider, SFT.mint));
