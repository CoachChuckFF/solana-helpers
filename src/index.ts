...
export const getSPLAccount = async (provider: Provider, mint: web3.PublicKey, vault?: web3.PublicKey, allowOffCurve?: boolean) => {
    let ata = vault ?? await getAssociatedTokenAddress(mint, provider.wallet.publicKey, allowOffCurve ?? false);
...
export const getSFTData = async (
    ...
    return {
        owner: owner,
        vault: await getAssociatedTokenAddress(mint, owner),
        mint: mint,
...
export const createSFT = async (
    ...
    let shareTally = 0;
    let hasOwner = false;
    for(var i = 0; i < metadataStruct.properties.creators.length; i++){
        let address = metadataStruct.properties.creators[i].address;
        let share = metadataStruct.properties.creators[i].share;
        ...
        if( address === owner.toString() ){ hasOwner = true; }
        shareTally += share;
    }
    ...
    const SFTData = await getSFTData(
        provider,
        splToken.mint,
        collectionData.collectionMint
    );
...
export const validateSFT = async(
